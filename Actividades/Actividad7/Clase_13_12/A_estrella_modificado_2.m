...
% START ALGORITHM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Optimal_path=A_estrella_modificado_2(MAPA, delta,pose_ini,pose_dest)
...
xTarget=floor(pose_ini(1)/delta)+1;%X Coordinate of the Target
yTarget=floor(pose_ini(2)/delta)+1;%Y Coordinate of the Target

MAPB(xTarget,yTarget)=0;%Initialize MAP with location of the target
plot(pose_ini(1),pose_ini(2),'rd');
%text(xval,yval,'Target')


% %The initial position
% xlabel('Please Select the the Target using the Left Mouse button ','Color','black');
% but=0;
% while (but ~= 1) %Repeat until the Left button is not clicked
%     [xval,yval,but]=ginput(1)
% end

xStart=floor(pose_dest(1)/delta)+1%Starting Position
yStart=floor(pose_dest(2)/delta)+1%Starting Position

MAPB(xStart,yStart)=1;
plot(pose_dest(1),pose_dest(2),'bo');
...
tstart=tic;
   ...
   Optimal_path=((Optimal_path_original-1)*delta)+(delta/2);
   Optimal_path=Optimal_path(2:end-1,:);
tfinal=toc(tstart) 
