...
[x(:,k+1),laser]=kuta(t(k+1),x,h,0,0,handles_plots,handles_coche,handle_vector,[45 45],MAPA);
 ...
 while (t0+h*k) < tf
    ...
    %------------------------------------
    velocidad=sqrt(componentes_x^2+componentes_y^2);  % es proporcional al vector para no pasar de velocidad en una curva o alg, con poco valor se choca 
    volante= 5*atan2(componentes_y,componentes_x);  %angulo de conduccion
