...
%ditancia de despegue y aterriza.
dd=50;
da=dd;
...
[x(:,k+1),laser]=kuta(t(k+1),x,h,0,0,handles_plots,handles_coche,handle_vector,[45 0],MAPA);
 ...
 rho=2*delta/LH^2;
  ...
  %velocidad(k)=(coordinador(k)*velocidad_traking(k))+((1-coordinador(k))*velocidad_reactiva(k));
  velocidad(k)=0.125*(coordinador(k)*velocidad_traking(k))+((1-coordinador(k))*velocidad_reactiva(k));
...
if volante(k)>pi/4 volante(k)=pi/4;
end
